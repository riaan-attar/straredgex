...
const SwarmCursor: React.FC<SwarmCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      // Return to center (the bulb atom) when not hovering
      let anchorX = cssW * 0.5;
      let anchorY = cssH * 0.5;
      
      if (cursor.has) {
        anchorX = cursor.x;
        anchorY = cursor.y;
      } else if (p.targetElementId) {
        const el = document.getElementById(p.targetElementId);
        if (el) {
          const rect = el.getBoundingClientRect();
          const r = container.getBoundingClientRect();
          anchorX = (rect.left + rect.width / 2) - r.left;
          anchorY = (rect.top + rect.height / 2) - r.top;
        }
      }
